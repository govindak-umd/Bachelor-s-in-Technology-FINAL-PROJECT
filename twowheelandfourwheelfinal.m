...
% reference: Prof Kannan C, lecture notes, Automotive Chassis, VIT Vellore, Jan-June 2016

clear all
clc

i=input('Enter: \n(1) for 2 wheel steering\n(2) for 4-wheel steering with crab steer\n(3) for 4-wheel steering with equal angles: ');
if i==1
    clear all
    clc
    ...
    bt=atan(Wb/(Wt+(Wb/tan(al))));          %front outer turn angle beta
    ...
    text((Wt+L)/2,(Wb/2),['\leftarrow Tcr = ',num2str(Tcr), ' units'],'FontWeight','bold','BackgroundColor',[1 1 1],'EdgeColor','black');
    hold off
...
elseif i==2
    ...
    %% taking inputs
    Wb=input('Enter Wheel Base: ');         %Wheel base
    Wt=input('Enter Wheel Track: ');        %Wheel track
    ...
    btf=atan((Wb-P.y)/P.x);
    %front outer turn angle beta_front
    ...
    %axis([xmin xmax ymin ymax])
    axis image
    text(((P.x)/2),P.y+((Wb-P.y)/2),['\leftarrow Tcr = ',num2str(Tcr), ' units'],'FontWeight','bold','BackgroundColor',[1 1 1],'EdgeColor','black');
    hold off
...
elseif i==3
    ...
    clear all
    clc
    disp('       FOUR WHEEL STEERING WITH EQUAL ANGLES');
    Wb=270;        %Wheel Base
    Wt=180;        %Wheel track
    al=input('Enter front inner wheel turn angle(in degrees, less than 50°): ');
    if Wb<=0||Wt<=0||al<=0||al>50||imag(Wb)~=0||imag(Wt)~=0||imag(al)~=0
    ...
    al=pi*al/180;                           %front inner turn angle alpha
    bt=acot(cot(al)+((2*Wt)/Wb));          %front outer turn angle beta
    %calculating all dimensions
    L=Wb/(2*tan(al));
    Tcr=(Wb/(2*sin(bt)));                       %Turning circle radius
    disp(['The turning circle radius is ',num2str(Tcr), ' units']);
    hold on
    grid off
    %plotting the car
    ...
    th=0.25*Wb/2;                           %tyre diameter, as a fraction of wheel base (aesthetics)
    %plotting rear wheels
    plot([-th*sin(bt); th*sin(bt)], [th*cos(bt); -th*cos(bt)], 'LineWidth', 5,'color','r')
    plot([Wt-th*sin(al); Wt+th*sin(al)], [th*cos(al); -th*cos(al)], 'LineWidth', 5,'color','r')
    %plotting dotted lines
    plot([Wt; Wt+L], [Wb; Wb/2], 'LineWidth', 1,'color','k','LineStyle','--')
    plot([Wt; Wt+L], [0; Wb/2], 'LineWidth', 1,'color','k','LineStyle','--')
    plot([0; Wt+L], [0; Wb/2], 'LineWidth', 1,'color','k','LineStyle','--')
    plot([0; Wt+L], [Wb; Wb/2], 'LineWidth', 1,'color','green','LineStyle','--')
    %plotting front wheels
    plot([-th*sin(bt); th*sin(bt)], [Wb-th*cos(bt); Wb+th*cos(bt)], 'LineWidth', 5,'color','r')
    plot([Wt-th*sin(al); Wt+th*sin(al)], [Wb-th*cos(al); Wb+th*cos(al)], 'LineWidth', 5,'color','r')
    %plotting the legend
    text((Wt)/2,(Wb/2)+(tan(bt)*(Wt/2 + L)),['\leftarrow Tcr = ',num2str(Tcr), ' units'],'FontWeight','bold','BackgroundColor',[1 1 1],'EdgeColor','black');
    hold off
